...
The Pune Municipal Corporation conducted a city-wide Tree Census in 2019, generating a large public dataset containing information on tree species, health condition, and spatial distribution across city wards.
While comprehensive, the raw dataset in its original form does not directly support actionable insights or policy-level decision making.

This project transforms the raw census data into a decision-support analytics solution using Python, SQLite, and Power BI, enabling city officials, urban planners, and civic stakeholders to monitor green cover, assess tree health, and proactively identify areas needing attention.

Objectives

The solution is designed to provide:

Operational visibility into tree distribution and health across wards
...
Large raw datasets are merged and cleaned using Python

Missing values and inconsistent categorical fields are handled

Analytical columns such as health indicators, risk levels, and monitoring status are derived

Ward-level summary tables are created for efficient analysis

Processed data is stored in a SQLite database, enabling direct integration with Power BI

The backend is modular and database-agnostic, and can be migrated to PostgreSQL for production-scale deployment.
...
Due to large file size constraints, the Power BI .pbix file could not be uploaded directly to GitHub.

📌 Power BI file (Google Drive link):
https://drive.google.com/file/d/1_WZsf_pKscxCdeGe-jI8smmCLRajqiQS/view?usp=drive_link

A PDF export of all dashboards is included in this repository for quick review.
